...
def GetAuthorizedCredentials(credential_path, env):
  """Builds authorized credentials used for talking to Google services.
  ...
  for a access token each time we need to auth, this can lead to us
  ...
  Returns:
    ...
    and issue authorized requests to Google services.
...
def _CopyCredentialsToTemp(src_path):
  ...
  # may not sufficiently lower the number of calls we issue to Google APIs.
