...
class ReplyFormatter(object):
  ...
  @classmethod
  def NdbTypeToGVizType(cls, ndb_type):
    """Returns the GViz type for a given NDB type.

    Args:
      ndb_type: An NDB property instance.

    Returns:
      The appropriate GViz DataTable type name.
    """
    if isinstance(ndb_type, ndb.IntegerProperty):
      return 'number'
    elif isinstance(ndb_type, ndb.FloatProperty):
      return 'number'
    elif isinstance(ndb_type, ndb.BooleanProperty):
      return 'boolean'
    elif type(ndb_type) in [
        ndb.StringProperty, ndb.TextProperty, ndb.ComputedProperty]:
      return 'string'
    elif isinstance(ndb_type, ndb.DateProperty):
      return 'date'
    elif isinstance(ndb_type, ndb.TimeProperty):
      return 'timeofday'
    elif isinstance(ndb_type, ndb.DateTimeProperty):
      return 'datetime'
    else:
      raise NotSupportedError('The property is not supported: ' + ndb_type)
